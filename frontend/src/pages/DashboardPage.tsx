...
interface Transaction { id: number; type: string; amount: string; counterparty_name: string; transacted_at: string; }
interface Summary { total_incoming: number; total_outgoing: number; net_position: number; transaction_count: number; incoming_count: number; outgoing_count: number; }
...
const ScoreGauge: React.FC<{ score: number; grade: string }> = ({ score, grade }) => {
  ...
  const color = { A: '#00A651', B: '#00A651', C: '#F59E0B', D: '#EF4444', F: '#EF4444' }[grade] || '#F59E0B';
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="mt-3 inline-block px-4 py-1 rounded-full text-white text-sm font-semibold" style={{ backgroundColor: color }}>
        Grade {grade}
      </span>
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-navy-900/5">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => { setOpen(false); setResult(null); setError(null); }}><X size={16} /></button>
      </div>
      ...
      <button onClick={handleParse} disabled={loading || !sms.trim()}
...
const OnboardingScreen: React.FC<{ onCreated: (b: Business) => void }> = ({ onCreated }) => {
  const [name, setName] = useState(''); const [type, setType] = useState('retail');
  const [industry, setIndustry] = useState(''); const [location, setLocation] = useState('');
  const [revenue, setRevenue] = useState(''); const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await createBusiness({ name, type, industry, location, monthly_revenue_estimate: revenue ? parseFloat(revenue) : undefined });
      onCreated(res.data);
    } catch (err: any) { setError(err.response?.data?.message || 'Could not create business.'); }
    finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-navy-900 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-paper rounded-2xl p-8 shadow-2xl">
          <h2 className="font-display text-2xl text-navy-900 mb-1">Karibu, {user?.name?.split(' ')[0]} 🎉</h2>
          <p className="text-navy-700 text-sm mb-8">Tuambie kuhusu biashara yako kuanza.</p>
          {error && <div className="bg-red-50 border border-kitu-red text-kitu-red text-sm rounded-lg px-4 py-3 mb-6">{error}</div>}
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              { label: 'Jina la Biashara', value: name, set: setName, placeholder: 'Jasiri General Store', required: true },
              { label: 'Sekta', value: industry, set: setIndustry, placeholder: 'Groceries, nguo, usafiri...' },
              { label: 'Mahali', value: location, set: setLocation, placeholder: 'Moshi, Kilimanjaro' },
            ].map((f) => (
              <div key={f.label}>
                <label className="block text-xs font-semibold text-navy-800 uppercase tracking-wider mb-2">{f.label}</label>
                <input value={f.value} onChange={(e) => f.set(e.target.value)} placeholder={f.placeholder} required={f.required}
            ...
            <div>
              <label className="block text-xs font-semibold text-navy-800 uppercase tracking-wider mb-2">Aina ya Biashara</label>
              <select value={type} onChange={(e) => setType(e.target.value)}
                className="w-full border border-navy-700/20 rounded-lg px-4 py-3 text-navy-900 bg-white focus:outline-none focus:ring-2 focus:ring-kitu-green text-sm">
                <option value="retail">Rejareja</option>
                <option value="vendor">Muuzaji</option>
                <option value="service">Huduma</option>
                <option value="agricultural">Kilimo</option>
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-navy-800 uppercase tracking-wider mb-2">Mapato ya Kila Mwezi (TZS)</label>
              <input value={revenue} onChange={(e) => setRevenue(e.target.value)} placeholder="500000" type="number"
                className="w-full border border-navy-700/20 rounded-lg px-4 py-3 text-navy-900 bg-white focus:outline-none focus:ring-2 focus:ring-kitu-green text-sm" />
            </div>
            <button type="submit" disabled={loading}
              ...
              {loading ? 'Inaandaa...' : 'Anza Biashara Yangu →'}
            </button>
          </form>
        </div>
      </div>
...
const DashboardPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'forecast' | 'network'>('overview');

  const loadDashboard = async () => {
    try {
      const bizRes = await getBusinesses();
      if (bizRes.data.length === 0) { setNoBusinessYet(true); return; }
      const biz = bizRes.data[0];
      setBusiness(biz);
      const [txRes, summaryRes, networkRes, forecastRes] = await Promise.all([
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      ...
      a.href = url; a.download = `kitu_report_${business.name}.pdf`; a.click();
      window.URL.revokeObjectURL(url);
    } finally { setLoadingReport(false); }
  };

  const handleLogout = async () => { await logout(); authLogout(); navigate('/login'); };

  const chartData = transactions.slice(0, 30).reverse().map((t) => ({
  ...
  const forecastChartData = forecast?.forecast.map((f) => ({
    date: new Date(f.date).toLocaleDateString('en-TZ', { day: 'numeric', month: 'short' }),
    flow: f.predicted_net_flow,
    positive: f.predicted_net_flow >= 0 ? f.predicted_net_flow : 0,
    negative: f.predicted_net_flow < 0 ? f.predicted_net_flow : 0,
  })) || [];

  if (loading) return (
    <div className="min-h-screen bg-paper flex items-center justify-center">
      <div className="font-display text-3xl text-navy-900 animate-pulse">Inapakia...</div>
    </div>
  );

  if (noBusinessYet) return <OnboardingScreen onCreated={(b) => { setBusiness(b); setNoBusinessYet(false); setLoading(false); }} />;
    ...
    <div className="min-h-screen bg-paper">
      ...
      <header className="bg-navy-900 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {business && (
            <button onClick={handleDownloadReport} disabled={loadingReport}
              className="flex items-center gap-2 text-paper/70 hover:text-paper text-xs transition-colors disabled:opacity-40">
              <Download size={14} />
              {loadingReport ? 'Inatengeneza...' : 'Ripoti ya PDF'}
            </button>
          )}
          <span className="text-paper/50 text-xs hidden md:block">{user?.name}</span>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <h2 className="font-display text-3xl text-navy-900">Habari, {user?.name?.split(' ')[0]} 👋</h2>
            {business && <p className="text-navy-700 text-sm mt-1">{business.name} · {business.location}</p>}
          </div>
          {business && <SmsWidget businessId={business.id} onSuccess={() => { setLoading(true); loadDashboard(); }} />}
        </div>

        {/* Score + Stats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-navy-900/5 flex flex-col items-center">
            <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-4">Alama ya Mkopo</p>
            {business?.latest_credit_score ? (
              <>
                ...
                <p className="text-xs text-navy-700 mt-4 text-center">
                  Uwezekano wa kulipa: <span className="font-semibold text-navy-900">{parseFloat(business.latest_credit_score.repayment_likelihood).toFixed(1)}%</span>
                </p>
              ...
              <div className="flex flex-col items-center gap-4 py-4">
                ...
                <p className="text-sm text-navy-700 text-center">Bado huna alama</p>
                <button onClick={handleRequestScore} disabled={loadingScore}
                  ...
                  {loadingScore ? 'Inahesabu...' : 'Pata Alama Yangu'}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="md:col-span-2 grid grid-cols-2 gap-4">
            {[
              { label: 'Jumla Inayoingia', value: summary?.total_incoming, count: summary?.incoming_count, color: 'text-kitu-green' },
              { label: 'Jumla Inayotoka', value: summary?.total_outgoing, count: summary?.outgoing_count, color: 'text-kitu-red' },
              { label: 'Nafasi ya Fedha', value: summary?.net_position, color: (summary?.net_position ?? 0) >= 0 ? 'text-kitu-green' : 'text-kitu-red' },
              { label: 'Miamala Yote', value: null, count: summary?.transaction_count, color: 'text-navy-900', label2: 'Miamala 90 iliyopita' },
            ].map((s, i) => (
              <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-navy-900/5">
                <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-3">{s.label}</p>
                {s.value !== undefined && s.value !== null ? (
                  <p className={`font-display text-2xl ${s.color}`}>TZS {s.value.toLocaleString()}</p>
                ) : (
                  <p className={`font-display text-2xl ${s.color}`}>{s.count ?? 0}</p>
                )}
                {s.count !== undefined && s.value !== undefined && s.value !== null && (
                  <p className="text-xs text-navy-700 mt-1">{s.count} miamala</p>
                )}
                {s.label2 && <p className="text-xs text-navy-700 mt-1">{s.label2}</p>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex gap-1 bg-white rounded-xl p-1 shadow-sm border border-navy-900/5 w-fit">
          {(['overview', 'forecast', 'network'] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 rounded-lg text-xs font-semibold uppercase tracking-wider transition-colors ${activeTab === tab ? 'bg-navy-900 text-paper' : 'text-navy-700 hover:text-navy-900'}`}>
              {tab === 'overview' ? 'Muhtasari' : tab === 'forecast' ? 'Utabiri' : 'Mtandao'}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'overview' && (
          <>
            {chartData.length > 0 && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-navy-900/5">
                <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-6">Mwenendo wa Fedha — Miamala 30 ya Hivi Karibuni</p>
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={chartData}>
                    ...
                    <YAxis tick={{ fontSize: 10, fill: '#243B55' }} tickLine={false} axisLine={false} tickFormatter={(v) => `${(v / 1000).toFixed(0)}K`} />
                    <Tooltip formatter={(value) => [`TZS ${Number(value).toLocaleString()}`, 'Kiasi']}
            ...
            {business?.latest_credit_score?.explanations && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-navy-900/5">
                <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-4">
                  Kwa Nini Alama Yako Ni {business.latest_credit_score.score}
                </p>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-navy-900/5">
              <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-4">Miamala ya Hivi Karibuni</p>
                  ...
                  <div key={t.id} className="flex items-center justify-between py-2 border-b border-paper last:border-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-navy-700">{new Date(t.transacted_at).toLocaleDateString('en-TZ', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                      </div>
                    </div>
        ...
        {activeTab === 'forecast' && forecast && (
          <>
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: 'Jumla Iliyotabiriwa (Siku 14)', value: `TZS ${forecast.summary.total_predicted_net_flow.toLocaleString()}`, color: forecast.summary.outlook === 'positive' ? 'text-kitu-green' : 'text-kitu-red' },
                { label: 'Wastani wa Kila Siku', value: `TZS ${forecast.summary.average_daily_flow.toLocaleString()}`, color: 'text-navy-900' },
                { label: 'Mwelekeo', value: forecast.summary.outlook === 'positive' ? '📈 Nzuri' : '📉 Angalia', color: forecast.summary.outlook === 'positive' ? 'text-kitu-green' : 'text-kitu-red' },
              ].map((s, i) => (
                <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-navy-900/5">
                  <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-2">{s.label}</p>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-navy-900/5">
              <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-6">Utabiri wa Fedha — Siku 14 Zijazo</p>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={forecastChartData}>
                  <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#243B55' }} tickLine={false} axisLine={false} />
                  <YAxis tick={{ fontSize: 10, fill: '#243B55' }} tickLine={false} axisLine={false} tickFormatter={(v) => `${(v / 1000).toFixed(0)}K`} />
                  <Tooltip formatter={(value) => [`TZS ${Number(value).toLocaleString()}`, 'Utabiri']}
                    contentStyle={{ borderRadius: '8px', border: '1px solid #E5E0D8', fontSize: '12px' }} />
                  <ReferenceLine y={0} stroke="#0D1B2A" strokeOpacity={0.2} />
                  <Bar dataKey="flow" radius={[4, 4, 0, 0]}>
                    {forecastChartData.map((entry, index) => (
                      <Cell key={index} fill={entry.flow >= 0 ? '#00A651' : '#EF4444'} fillOpacity={0.8} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-navy-900/5">
              <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-4">Msimu na Matukio</p>
              <div className="space-y-2">
                {forecast.forecast.map((f, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-paper last:border-0">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${f.predicted_net_flow >= 0 ? 'bg-kitu-green' : 'bg-kitu-red'}`} />
                      <p className="text-sm text-navy-900">{new Date(f.date).toLocaleDateString('en-TZ', { weekday: 'short', day: 'numeric', month: 'short' })}</p>
                    </div>
                    <div className="flex items-center gap-6">
                      <span className="text-xs text-navy-700">Msimu: ×{f.seasonal_multiplier}</span>
                      <p className={`text-sm font-semibold ${f.predicted_net_flow >= 0 ? 'text-kitu-green' : 'text-kitu-red'}`}>
                        {f.predicted_net_flow >= 0 ? '+' : ''}TZS {f.predicted_net_flow.toLocaleString()}
                      </p>
                    </div>
                  </div>
                ))}
        ...
        {activeTab === 'network' && network && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: 'Washirika Wote', value: network.unique_counterparties, icon: <Users size={18} /> },
                { label: 'Washirika wa Kuamini', value: network.loyal_counterparties, icon: <Users size={18} /> },
                { label: 'Afya ya Mtandao', value: `${network.network_health_score}%`, icon: <Globe size={18} /> },
                { label: 'Viungo vya Mtandao', value: network.total_nodes, icon: <Globe size={18} /> },
              ].map((s, i) => (
                <div key={i} className="bg-white rounded-2xl p-5 shadow-sm border border-navy-900/5">
                  <div className="text-kitu-green mb-2">{s.icon}</div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-navy-900/5">
              <p className="text-xs font-semibold text-navy-700 uppercase tracking-widest mb-4">Washirika Wakuu kwa Kiasi</p>
              <div className="space-y-3">
                {network.top_counterparties.map((c, i) => {
                  const maxVol = network.top_counterparties[0].total_volume;
                  const pct = (c.total_volume / maxVol) * 100;
                  return (
                    <div key={i} className="space-y-1">
                      <div className="flex justify-between text-sm">
                        <span className="text-navy-900 font-medium">{c.phone}</span>
                        <span className="text-navy-700">TZS {c.total_volume.toLocaleString()}</span>
                      </div>
                      <div className="h-2 bg-paper rounded-full overflow-hidden">
                        <div className="h-full bg-kitu-green rounded-full transition-all duration-700"
                          style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                  );
